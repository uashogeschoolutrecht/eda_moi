...
The data would need to include at least:
 ...
 - The age at dead of each victim
...
To load the data into R you can run the following command. The data are in csv format: 
After running the command below the Shipman murders dataset will be stored in 
in the Global Environment of your R-session as `data.frame` called `shipman_murders`
You can access this variable by typing it's name in the `Console` or inspecting it in the RStudio interactive viewer under the `Environment` tab. 
...
If we review the list above we can already check a number of items from the list that we have taken care off. Let's take a look at the data first

```{r}
DT::datatable(shipman_murders)
```

If you are viewing this book in a browser you should see an interactive table. Take 2 minutes to study the data and 2 additional minutes to discuss what you see with one of your fellow course participants.
...
The data consists of 10 columns (called `variables` in R lingo) and 215 rows (or sometimes called `observations` or in this case `subjects`)
...
One important condition could be that there are now duplicated names in the dataset. If we want to confirm there are not, we could take a look at all 215 records and try to remember if we see duplicated names. If we look at this list often enough, this would problably work, but takes quite some time and is error prone.
...
The following code:
 ...
 1. Turns all names in the now `name` variable into lowercase.
...
Let's conclude that indeed there are 215 unique names in the dataset and that no duplicates exist. So we can answer the first question on how many people Shipman killed with `215` people. This number was mentioned on the Wiki as well. (_Can you think of an additional check for duplicated names before we move on?_) 
...
We will first look at the other 2 questions remaining and at the end of this chapter, I will summarize some explicit assumptions to the conclusion of the complete analysis.

## Let's investigate what the gender distribution was of the victims and what their ages were
...
From the little table above we can concluded that Shipman indeed murdered more females than males, about `r round(178/37, digits = 0)` times as much  
...
Firstly, only those of the females and secondly all victims. The dashed blue line indicates the median age, the solid blue line indicates the mean. The graph shows a frequency polynoma and a 
