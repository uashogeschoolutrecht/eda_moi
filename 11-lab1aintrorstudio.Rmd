...
A) Try to load the datafile "/data/kiemingen.xlsx" the way described above.
...
## **EXERCISE 2** Appearance and general settings

A) Try adjusting the appearance of you RStudo so that it matches mine (I use the theme `Merbivore`)

B) In the `global options` unselect the option "Show output inline for all Rmd documents" in the Rmd panel.  
