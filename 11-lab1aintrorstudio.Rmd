...
## RStudio IDE
Below we will show you a few tricks to be more productive with the RStudio IDE software. Figure \@ref(fig:rstudiointerface) shows the RStudio Interface on my computer while writing this website. The four windows that we use to work interactively with R are indicated by the big yellow numbers 1 to 4.
...
## The RStudio IDE Windows. 
 ...
 ``` 
 Try opening a new R-Script by clicking `File` -> `New File` -> `R Script`. The new file will open up in the top-left window.
...
## Restarting R 
If you are really stuck it is possible to restart R. If R hangs, restart R by: `Session` -> `Restart R`
