...
## Using the Normal distribution to calculate probability of observing a specific value or value range

Before we actually start with the calculating probabilities we first calculate the mean $\mu$, and the standard deviation, $\delta$, of the target variable. Hence, we standardize the variable to get a standard normal distribution with $\mu$ = 0 and $\delta$ = 1
and assign it to an appropriate variable name.
...
height.sd <- sd(females$height)
height.sd

## [1] 7.919726

# z-transformation
height.z <- (females$height - height.mean)/height.sd

The height variable has a mean of 163.7 cm of and a standard deviation of 7.9 cm.




## EDA on the 'students' data
...
## PCA and clustering
see also: https://dev.to/juliasilge/pca-and-umap-with-cocktail-recipes-4980
