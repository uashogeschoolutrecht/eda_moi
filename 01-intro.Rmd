...
## Hier wil je misschien een rm(list=ls()) doen om de omgeving schoon te maken
...
(ref:eda) The process of Exploratory Data Analysis (EDA) is not a formal strict process. It involves interactive cycles of `loading`, `cleaning`, `wrangling`, `visualizing`, `communicating` data and patterns in the data. The process of EDA is directed towards gaining insight in the data in basically any way you can. It can involve a number of statistical approaches that are sometimes collectively called IDA (Initial Data Analysis), which is the the description and check if underlying assumptions for any formal statistical modelling are met. Usually the formal statistical inference is out of scope of the EDA process, although we will see examples in which statistical modelling can help us perform EDA better. Because it is a not-so-strict iterative process there is no formal manual on which steps to perform in the EDA process. You could consider doing EDA more as being in a certain state-of-mind. To help you overcome this rather abstract way of looking at this process, several authors have created a check list to aid performing EDA in a more structured way. Here I will go over such a check list, but bear in mind that is only an aid and no formal manual: __When doing EDA you should always keep an open mind to deviate from the checklist, skip a check-box or add one yourself.__
...
This course will not be a statistics course!! So you can relax a bit more now. BUT: we will need some statistical principles and insights to succesfully exploit the complete EDA process. EDA is all about providing insignt into data, most of the time you will know very little about it on beforehand. Statistics can be very helpful to find and assess patterns in a more formal way. During this course we will avoid in-depth explanations about statistical models and we will use as few formulas as possible. Most of the time, we focus on exploring assumptions that are a prequisite for perfoming statistics. 

Sometimes, we will also looks at more complex examples like for example regressions, regression tree methods like Random Forest or Boosting and Analysis of Variance methods. This is mainly meant as demonstrations how to build upon EDA with modelling and to illustrate how model performance and inferential results can be obtained and read.
...
### **EXERCISE** {-}
Using the function `installed.packages()` and running the code below. Try to figure out how many packages are installed on the RSudio server instance that you have logged into.
