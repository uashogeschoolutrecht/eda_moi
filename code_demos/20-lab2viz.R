...
## ------------------------------------------------------------------------
naniar::vis_miss(gapminder) + citrulliner::theme_individual() + toolboxr::rotate_axis_labels(axis = "x", angle = 90)
...
## or combined with treansparancy
gapminder %>% 
  ggplot(aes(x = fertility,
             y = life_expectancy)) +
...
summarize_plot <- gapminder %>% 
  ...
  geom_point(aes(colour = continent, shape = as_factor(year)), alpha = 0.7) 
...
## ---- all_data_airq, options_exercises-----------------------------------
names(airq)
plot_1c <-  airq %>%
...
## ---- overplotting_airq, options_exercises-------------------------------
names(airq)
plot_1d <-  airq %>%
  ggplot(aes(x = month, y = wind)) +
...
## ---- line_wind_month, options_exercises---------------------------------
names(airq)
plot_1e <-  airq %>%
  ggplot(aes(x = month, y = wind)) +
  geom_point(aes(), alpha = 0.4, position = "jitter")  +
...
## ---- incidence_tb, options_exercises------------------------------------
tb_data_selected %>%
  as_tibble() %>%
...
## ---- highest_tb_incidence, options_exercises----------------------------
tb_data_selected %>%
  as_tibble() %>%
  mutate(incidence_1e3 = (e_inc_num/e_pop_num)*1000) %>%
  dplyr::filter(incidence_1e3 > 5,
...
## ---- average_inc_tb, options_exercises----------------------------------
