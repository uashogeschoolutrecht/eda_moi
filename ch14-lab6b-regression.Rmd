...
offset_americas <- get_regression_table(lifeExp_model) %>%
  dplyr::filter(term == "continent: Americas") %>%
  pull(estimate) %>%
  round(1)
...
offset_asia <- get_regression_table(lifeExp_model) %>%
  dplyr::filter(term == "continent: Asia") %>%
  pull(estimate) %>%
  round(1)
mean_asia <- intercept + offset_asia

## 1. contrast handmatig te definieren dmv matrix
## 2. handmatig de volgorde van de levels in de factor

# Europe
offset_europe <- get_regression_table(lifeExp_model) %>%
  dplyr::filter(term == "continent: Europe") %>%
  pull(estimate) %>%
  round(1)
...
offset_oceania <- get_regression_table(lifeExp_model) %>%
  dplyr::filter(term == "continent: Oceania") %>%
  pull(estimate) %>%
  round(1)
