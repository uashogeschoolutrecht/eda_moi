# Lab 2: Visualize & Explore Data {#lab2viz}
...
**The EDA visualization stages:** 
 ...
 1. `Characteristics`; Variable types, number of data points, patterns, grouping information etc. Usually this is done by creating plots showing all data available or a random sample if you have a lot of data. Creating plots for missing values can be a good first step. A next step is creating scatter plots.
 1. `Distributions`; By looking at distributions you can generate hypothesis about the nature of sampling of the data, whether there are outliers, whether distributions are skewed or whether a transformation is feasible
 1. `Outliers`; No matter what you hear about removing outliers, never do it without solid understanding of your data and the reason why an outlier exists. Outliers can be discovered using box-plots, distribution plots or scatter plots (individual points)
 1. `Sorting`; There can be a logical hierarchy in the data when variables are sorted according others
 1. `Trends`; Trends sometimes require you to subset the data or generate summary data based on grouping variables. This means that part of the data is not visible anymore, but if performed well, it will also reveal new information. Removing data for the sake of summarizing and keeping data for the sake of completeness are in delicate balance. Again this is an iterative process 
 1. `Models`; Once hypothesis are generated and patterns have been identified it is time to try some models on the data. Visualizing models and their parameters is part of checking model quality and fit to the data 
...
This is a busy graph and for this Gapminder dataset not so informative but parts of it can be useful as a next step to zoom in at with additional graphs. In Chapter \@ref(eda) we will see further examples of automated EDA. 
...
This is a very dense plot showing 'overplotting. We can fix this in several ways:
 ...
 1. Displaying a model / relationship that represents the data (and not show the actual data itself) 
...
Add model to plot
In chapter \@ref(assumptionsmodels) we will see how to handle multiple models. For instance: imagine you want to creat a model for each country separately over time. Can you think of an application for such models?
```{r}
...
What happened in 1999/2010? The data shows a dip.
```{r}
gapminder$continent %>% as_factor() %>% levels()
gapminder %>% 
  dplyr::filter(continent == "Americas" | continent == "Oceania") %>%
...
In such cases it can be helpful to add some noise to the points (`position = "jitter"`) to reduce overlapping. This can be a powerful approach, especially when combined with setting `alpha()`
```{r}
gapminder %>% 
  dplyr::filter(continent == "Americas" |
                continent == "Africa") %>%
...
Now let's plot a new distribution plot, this time we plot density, excluding the outliers.
```{r}
heights_data %>%
  dplyr::filter(height < 100) %>%
  ggplot(aes(height)) +
...
### `{ggplot2}` {-}
As shown in the demo, the {ggplot2} package is a very strong tool to make plots. It is somewhat more difficult than other plotting systems in R, but it is much stronger and much, much more versatile.
...
You will see that the plot contains many points that are overlaid. How can you solve this "overplotting" problem? Write a few lines of code that shows your solution.

**TIPs**

 - Look-up overplotting in the "R for Data Science" book (review Chapter \@ref(lab2viz))
 - What does `alpha()` do?
...
#### 1F) Wind Velocity {-}
summarize the wind velocity per month (call this `mean_month`), plot the relationship between `month` and `mean_wind`.


### 2. Overplotting solved by colours {-}
...
#### 2B) Selection of relevant variables
...
Use `dplyr::filter()` together with the `%in%` operator to pull out the descriptions of the variables from the data dictionary. Do all the variables have a description?
...
#### 3B) Calculate incidence per 1.000 {-}
...
#### 5B) Turn the code of the histograms into code that plots frequency polynomes
Maybe do a log transformation on the incidence and/or gdp? Use facets for the years.
...
#### 6B) Change all variable names to lower type case {-}
...
### 6C) Scatter plot of all the data {-}
...
#### 6E) Overplotting solved by colours {-}
...
#### 6F) Combining "jitter" with group colours {-}
...
#### 6G) Solving overplotting by reducing data dimensionality: summarizing data {-}
...
### 7. Excel files - load and visualize {-}
 
Loading Microsoft Office Excel files. 

#### 7A) Loading an Excel file {-}
 
The data for this exercise can be found in:
`"./data/Animals.xls"`

It is an Microsoft Excel file containing brain and body weight of a number of animals: http://mste.illinois.edu/malcz/DATA/BIOLOGY/Animals.html 
...
**TIPs** 
 ...
 - The variables in this dataset need to be `animal`, `body_weight` and `brain_weight` and IN THAT ORDER! These are the so-called `names` of the dataframe and can be set or checked using the function `names()`
...
#### 7B) Variable names {-}
The `names` of a dataframe can be found or set with `names(dataframe)`. Try setting the names for the `animal` dataframe to `animal`, `body_weight` and `brain_weight` and IN THAT ORDER. 
...
#### 7E) Plots {-} 
Create a plot that shows the realtionship between `body_weight` and `brain_weight`. Create a dot plot, that shows this relationship.
...
#### 7H) What can you conclude from the 2G plot {-}
What happens to the relation between body_weight and brain_weight if you excluse all dinosaurs ("Brachiosaurus", "Diplodocus" and "Triceratops") from the dataset?
