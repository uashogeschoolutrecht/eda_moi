...
## **EXERCISE 5** Wide to long; Spreading and Gathering {-}
...
## **EXERCISE 6** Additional operations {-}
...
### **Exercise 8** Some questions about the WHO case demo

8A) In this case study we set `na.rm = TRUE` just to make it easier to
...
8B) What happens if you neglect the `mutate()` step?
    (`mutate(key = stringr::str_replace(key, "newrel", "new_rel"))`)

8C) We claimed that `iso2` and `iso3` were redundant with `country`. 
    Confirm this claim.

8D) For each country, year, and sex compute the total number of cases of 
