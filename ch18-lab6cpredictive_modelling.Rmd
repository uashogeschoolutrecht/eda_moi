# Lab 6b; Predictive modelling {#lab6bpredictive_modelling}
...
and information about buidling models with `{caret}`: https://www.machinelearningplus.com/machine-learning/caret-package/#35howtopreprocesstotransformthedata
...
**The thing you need to take away from this exercise/demo is that it is most important to never ever let your models be exposed to data from the test set, before it it time to test the model**. The problems that arise when you omit to take this rule into account are comparable to insider trading. In most cases it will not put you in jail, but it could...
...
train %>%
  ...
  geom_bar(position = "dodge") + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) + 
  ggtitle("Survival by title and Pclass") + facet_wrap(~ reorder(Pclass))
...
train %>%
  ...
  facet_wrap(~ reorder(Pclass))

train %>% ungroup %>% count(Fare.grps)
```

## Pclass
The `Pclas` variable can be regarded a proxy for socio-economical class. 1 is higher class (more expensive ticket), 2 is middle class, (somewhat cheaper) and 3 is lower class (cheap ticket).
